...
def load_dual_data(rgb_dir, mask_dir, target_size=(300, 300), batch_size=32):
    ...
    while True:
        rgb_batch, labels = next(rgb_generator)  # 수정된 부분
        mask_batch, _ = next(mask_generator)    # 수정된 부분
        yield [rgb_batch, mask_batch], labels


train_generator = load_dual_data(train_rgb_dir, train_mask_dir)
val_generator = load_dual_data(val_rgb_dir, val_mask_dir)

# 클래스 이름 저장
class_names = sorted(os.listdir(train_rgb_dir))
...
# 테스트 예측
test_rgb_path = 'lowIBirdMGClassification/resource/test/images/sample_bird.jpg'
